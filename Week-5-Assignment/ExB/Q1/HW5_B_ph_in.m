...
% inout phases
ph_in=[0 0 0]; %[rad]
ph_in_range = 0:0.01:1;
F1 = zeros(length(ph_in_range), 0);
F2 = zeros(length(ph_in_range), 0);
F3 = zeros(length(ph_in_range), 0);
...
for i = 1:length(ph_in_range)
    % input signal
    in=A(1)*cos(2*pi*fin(1)*t+ph_in_range(i))+...
...
subplot(3,1,1)
plot(ph_in_range,F1)
xlabel('Phase of signal')
ylabel('Q1 Fidelity')
legend('Q1 VS Phase of signal')
xlim([0 1])
grid
