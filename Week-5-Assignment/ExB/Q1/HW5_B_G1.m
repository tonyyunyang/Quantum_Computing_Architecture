...
% inout phases
ph_in=[0.31 0 0]; %[rad]
G1_coefficient = 0:0.025:10;
F1 = zeros(length(G1_coefficient), 0);
F2 = zeros(length(G1_coefficient), 0);
F3 = zeros(length(G1_coefficient), 0);
...
for i = 1:length(G1_coefficient)
    ...
    G1=G1_coefficient(i)/sinc(fin(1)/fsample); %[-]
...
figure
subplot(3,1,1)
plot(G1_coefficient,F1)
xlabel('G1 coefficient')
ylabel('Q1 Fidelity')
legend('Q1 VS G1')
xlim([0 10])
grid
