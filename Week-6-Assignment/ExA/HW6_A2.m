...
I_new = I .* cos(fosc*t) + Q .* sin(fosc*t);
Q_new = Q .* cos(fosc*t) + I .* sin(fosc*t);
...
I_sampled=I_new(1:1/(fsample*dt):end);

%sampling the input signal Q
Q_sampled=Q_new(1:1/(fsample*dt):end);

%quantize the signal on N-bit
%first, scale the signal to fit in [0:2^N-1] range, then round it
D_I=round((I_sampled-min(I_new))/(max(I_new)-min(I_new))*(2^N-1));
D_Q=round((Q_sampled-min(Q_new))/(max(Q_new)-min(Q_new))*(2^N-1));
