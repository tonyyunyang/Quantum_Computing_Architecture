...
phase_shift = 0:0.01:3.14;


F1 = zeros(length(phase_shift), 0);
F2 = zeros(length(phase_shift), 0);
...
for i = 1:length(phase_shift)
    ...
    % apply filter to VoutDAC_I
    VoutDAC_I_filtered = filter(b, a, VoutDAC_I);
    
    % apply filter to VoutDAC_Q
    VoutDAC_Q_filtered = filter(b, a, VoutDAC_Q);
    
    % figure
    % plot(t(1:1000),VoutDAC_I_filtered(1:1000),t(1:1000),(I(1:1000)-min(I))/(max(I)-min(I))*DAC_or-DAC_or/2);
    % xlabel('Time [s]')
    % ylabel('Amplitude [-]')
    % legend('DAC_I input','ideal voltage (scaled for comparison)')
    % 
    % figure
    % plot(t(1:1000),VoutDAC_Q_filtered(1:1000),t(1:1000),(Q(1:1000)-min(Q))/(max(Q)-min(Q))*DAC_or-DAC_or/2);
    % xlabel('Time [s]')
    % ylabel('Amplitude [-]')
    ...
    I_mix = 2 * VoutDAC_I_filtered .* cos(2 * pi * fosc * t);
    Q_mix = 2 * VoutDAC_Q_filtered .* sin(2 * pi * fosc * t);
