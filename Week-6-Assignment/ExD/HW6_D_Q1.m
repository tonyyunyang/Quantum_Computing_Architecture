...
phase_error_range = 0:0.01:3.14;
F1 = zeros(length(phase_error_range), 0);
F2 = zeros(length(phase_error_range), 0);


for i = 1:length(phase_error_range)
    disp(i);
    phase_error = phase_error_range(i); % Phase error in radians
    amplitude_error = 0; % Amplitude error
    I_mix = (1 + amplitude_error) * 2 * VoutDAC_I .* cos(2 * pi * fosc * t + phase_error);
    Q_mix = (1 + amplitude_error) * 2 * VoutDAC_Q .* sin(2 * pi * fosc * t + phase_error);
    Mix_out = I_mix + Q_mix;
...
figure
subplot(2,1,1)
plot(phase_error_range,F1)
xlabel('Phase Error')
ylabel('Q1 Fidelity')
legend('Q1 VS Phase Error')
grid

subplot(2,1,2)
plot(phase_error_range,F2)
xlabel('Phase Error')
ylabel('Q2 Fidelity')
legend('Q2 VS Phase Error')
grid