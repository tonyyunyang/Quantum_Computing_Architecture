...
error_range = 0:0.01:1;
F1 = zeros(length(error_range), 0);
F2 = zeros(length(error_range), 0);


for i = 1:length(error_range)
    disp(i);
    phase_error = 0; % Phase error in radians
    amplitude_error = error_range(i); % Amplitude error
...
subplot(2,1,1)
plot(error_range,F1)
xlabel('Amplitude Error')
ylabel('Q1 Fidelity')
legend('Q1 VS Amplitude Error')
grid
