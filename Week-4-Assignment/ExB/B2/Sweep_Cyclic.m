...
Gtemp3_range = 0:50:10000;
Gtemp4_range = 0:.05:10;
...
F2_G3 = zeros(length(Gtemp3_range), 0);
F2_G4 = zeros(length(Gtemp4_range), 0);
F3_G3 = zeros(length(Gtemp3_range), 0);
F3_G4 = zeros(length(Gtemp4_range), 0);
...
for i = 1:length(Gtemp3_range)
    ...
    F2_G3(i)=probabilities(3,end,2);
    F3_G3(i)=probabilities(3,end,3);
end
...
for j = 1:length(Gtemp4_range)
    ...
    % be simply computed by measuring the qubit along Z. This measurmenet is
    % insensitive with respect to rotazion around Z.
    F2_G4(j)=probabilities(3,end,2);
    F3_G4(j)=probabilities(3,end,3);
end

%we plot four seperate graphs below
figure;
plot(Gtemp3_range, F2_G3);
title('Qubit 2 Fidelity VS G2.2');
xlabel('G2.2');
ylabel('Q2 Fidelity');

figure;
plot(Gtemp3_range, F3_G3);
title('Qubit 3 Fidelity VS G2.2');
xlabel('G2.2');
ylabel('Q3 Fidelity');

figure;
plot(Gtemp4_range, F2_G4);
title('Qubit 2 Fidelity VS G2.3');
xlabel('G2.3');
ylabel('Q2 Fidelity');

figure;
plot(Gtemp4_range, F3_G4);
