...
Gtemp1_range = 280:.05:290;
Gtemp2_range = 28450:.25:28500;
%Gtemp3_range = 800:0.1:830;
%Gtemp4_range = 161000:1:161500;
...
F3_Gtemp1 = zeros(length(Gtemp1_range), 0);
F3_Gtemp2 = zeros(length(Gtemp2_range), 0);
%F3_Gtemp3 = zeros(length(Gtemp3_range), 0);
%F3_Gtemp4 = zeros(length(Gtemp4_range), 0);
...
for i = 1:length(Gtemp1_range)
    ...
    F3_Gtemp1(i)=probabilities(3,end,3);
end
...
for j = 1:length(Gtemp2_range)
    ...
    % be simply computed by measuring the qubit along Z. This measurmenet is
    % insensitive with respect to rotazion around Z.
    F3_Gtemp2(j)=probabilities(3,end,3);
end
...
%     % be simply computed by measuring the qubit along Z. This measurmenet is
%     % insensitive with respect to rotazion around Z.
%     F3_Gtemp3(i)=probabilities(3,end,3);
% end
% 
% % start the second loop to collect all possible F3
% for j = 1:length(Gtemp2_range)
...
%     % be simply computed by measuring the qubit along Z. This measurmenet is
%     % insensitive with respect to rotazion around Z.
%     F3_Gtemp4(j)=probabilities(3,end,3);
% end

% we plot two seperate graphs below
figure;
plot(Gtemp1_range, F3_Gtemp1);
title('Qubit 3 Fidelity VS G1.2');
xlabel('G1.2');
ylabel('Q3 Fidelity');

figure;
plot(Gtemp2_range, F3_Gtemp2);
